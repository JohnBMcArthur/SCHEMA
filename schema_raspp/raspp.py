...
def _column_unanimous_across_parents(parents, col_idx):
	"""True if every parent has the same character at alignment column col_idx."""
	if col_idx < 0 or col_idx >= len(parents[0]):
		return False
	ref = parents[0][col_idx]
	return all(p[col_idx] == ref for p in parents)


def crossover_1based_unanimous_adjacent_columns(parents, xover_1based_full):
	"""
	True if SCHEMA 1-based crossover xover_1based_full lies between two full
	alignment columns that are each unanimous across parents (e.g. all A then all R).

	A SCHEMA crossover value c splits after full 0-based column c-1 and before c.
	"""
	left = xover_1based_full - 1
	right = xover_1based_full
	return (
		_column_unanimous_across_parents(parents, left)
		and _column_unanimous_across_parents(parents, right)
	)


def allowed_collapsed_starts_unanimous_adjacent_columns(parents, collapsed_parents, identical_sites):
	"""
	0-based collapsed indices s where a fragment may start (second fragment starts at s)
	so that the corresponding full-alignment SCHEMA crossover only occurs between two
	unanimous adjacent columns.

	identical_sites is the list returned by collapse_parents (full column indices removed);
	a sorted copy is passed to translate_collapsed_indices (which mutates its sites list).
	"""
	L = len(collapsed_parents[0])
	if L < 2:
		return set()
	sites_work = sorted(identical_sites)
	allowed = set()
	for s in range(1, L):
		xb = [s + 1]
		translate_collapsed_indices(xb, list(sites_work))
		if crossover_1based_unanimous_adjacent_columns(parents, xb[0]):
			allowed.add(s)
	return allowed


class RASPPPrepared(object):
	"""Cached inputs for repeated RASPP_SCHEMA solves (multi-fragment sweeps)."""

	__slots__ = ("parents", "collapsed_parents", "identical_sites", "arc_lengths")

	def __init__(self, parents, collapsed_parents, identical_sites, arc_lengths):
		self.parents = parents
		self.collapsed_parents = collapsed_parents
		self.identical_sites = identical_sites
		self.arc_lengths = arc_lengths


def RASPP_prepare(contacts, parents):
	"""
	Build arc lengths and collapsed alignment once; reuse with RASPP_solve_core
	for many num_crossovers values (same contacts, parents, alignment).
	"""
	...
	return RASPPPrepared(parents, collapsed_parents, identical_sites, arc_lengths)


def _translate_raspp_results(results, identical_sites):
	"""Map crossover indices from collapsed alignment back to full parent coordinates."""
	sites_copy = sorted(identical_sites)
	for i in range(len(results)):
		(avg_E, collapsed_crossovers, l_min, l_max) = results[i]
		crossovers = translate_collapsed_indices(collapsed_crossovers, list(sites_copy))
		results[i] = (avg_E, crossovers, l_min, l_max)
	return results


def RASPP_solve_core(
	collapsed_parents,
	arc_lengths,
	num_crossovers,
	min_fragment_diversity,
	allowed_fragment_starts=None,
):
	"""
	Dynamic-programming search only. Crossovers are in collapsed coordinates;
	call _translate_raspp_results to map to full alignment indices.

	allowed_fragment_starts: if not None, each internal fragment must start at a
	0-based collapsed index in this set.
	"""
	num_residues = len(collapsed_parents[0])
	if (num_crossovers+1)*min_fragment_diversity > num_residues:
...
def RASPP_SCHEMA_from_prepared(
	prepared,
	num_crossovers,
	min_fragment_diversity,
	allowed_fragment_starts=None,
):
	"""Run RASPP_SCHEMA for one crossover count using a prior RASPP_prepare()."""
	results = RASPP_solve_core(
		prepared.collapsed_parents,
		prepared.arc_lengths,
		num_crossovers,
		min_fragment_diversity,
		allowed_fragment_starts=allowed_fragment_starts,
	)
	return _translate_raspp_results(results, prepared.identical_sites)
...
def RASPP_SCHEMA(
	...
	prepared = RASPP_prepare(contacts, parents)
	return RASPP_SCHEMA_from_prepared(
		prepared,
		num_crossovers,
		min_fragment_diversity,
		allowed_fragment_starts=allowed_fragment_starts,
...
def RASPP(avg_energies, parents, num_crossovers, min_fragment_diversity, allowed_fragment_starts=None):
	...
	return _translate_raspp_results(results, identical_sites)
...
def get_shortest_path(
	...
	for i in range(num_residues):
		...
		if l_min <= length <= l_max:
			if (
				num_crossovers >= 1
				and allowed_fragment_starts is not None
				and i not in allowed_fragment_starts
			):
				continue
	...
	for k in range(1, num_crossovers):
		...
		for j in range(min_j, max_j):
			if allowed_fragment_starts is not None and j not in allowed_fragment_starts:
				continue
